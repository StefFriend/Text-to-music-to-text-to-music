...
def clean_text(text):
    phrases_to_remove = [
        ...
        "The tempo is",
        "The music is",
        "The low quality recording features",
        "It sounds",
        "This is a",
        "This audio contains"
    ]
...
def captioning(args):
    ...
    #    #item = {"text":text}
    #    inference[chunk] = text
    #    print(text)


    #CAPSTONE IMPLEMENTATION
    for chunk, text in zip(number_of_chunks, output):
        cleaned_text = clean_text(text)  # Clean the text to remove specified phrases
        #time = f"{chunk * 10}:00-{(chunk + 1) * 10}:00"
        inference[chunk] = {"text": cleaned_text}  # Store cleaned text with time in dictionary
